...
@section('title', 'Tambah Kategori - StoreKuify')
...
<div x-data="{ sidebarOpen: false }" class="h-screen flex overflow-hidden bg-slate-50">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <!-- Header -->
        @include('partials.header', ['breadcrumbs' => [['label' => 'Beranda', 'url' => route('owner.dashboard')], ['label' => 'Data Barang', 'url' => route('owner.barang')], ['label' => 'Tambah Kategori']]])
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50">
            <div class="max-w-2xl mx-auto space-y-6">
                <!-- Heading -->
                <div>
                    <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">Tambah Kategori Baru</h2>
                    <p class="text-slate-500 text-xs mt-0.5">Silakan buat kategori barang baru untuk warung Anda.</p>
                </div>

                <!-- Form Card -->
                <div class="bg-white border border-slate-200/60 rounded-2xl p-6 sm:p-8 shadow-sm">
                    <form action="{{ route('owner.barang.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                ...
                                placeholder="Keterangan opsional tentang kategori ini">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-xs font-semibold text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
