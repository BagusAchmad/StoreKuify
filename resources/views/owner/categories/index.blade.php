...
@section('title', 'Kategori Barang - StoreKuify')
...
<div x-data="{ sidebarOpen: false, showDeleteModal: false, deleteActionUrl: '' }" class="h-screen flex overflow-hidden bg-slate-50">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <!-- Header -->
        @include('partials.header', ['breadcrumbs' => [['label' => 'Beranda', 'url' => route('owner.dashboard')], ['label' => 'Data Barang']]])
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight mt-1.5">Kategori Barang</h2>
                </div>
                
                <a href="{{ route('owner.barang.create') }}" class="inline-flex items-center gap-2 bg-[#1e5cfb] hover:bg-[#1a52db] text-white px-5 py-3 rounded-xl text-sm font-bold shadow-md shadow-blue-500/10 hover:shadow-blue-500/20 active:scale-[0.98] transition duration-150 cursor-pointer">
            ...
            @if($errors->has('category_delete'))
                <div class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-600 text-sm font-semibold flex items-center gap-3">
                    ...
                    <span>{{ $errors->first('category_delete') }}</span>
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="bg-white border border-slate-200/60 rounded-2xl p-6 flex flex-col justify-between shadow-sm">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <span class="text-2xl font-extrabold text-slate-800 block mt-2">{{ $lowStockCount }}</span>
                        </div>
                ...
                <!-- Card 3: Estimasi Total Nilai Barang -->
                <div class="bg-white border border-slate-200/60 rounded-2xl p-6 flex flex-col justify-between shadow-sm">
                    <div class="flex justify-between items-start">
                        <div>
                            <span class="text-xs font-bold text-slate-400 block tracking-wide uppercase">Estimasi Total Nilai Barang</span>
            ...
            <div class="bg-white border border-slate-200/60 rounded-2xl shadow-sm overflow-hidden flex flex-col justify-between">
                ...
                <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
                    <!-- Search Input -->
                    <form action="{{ route('owner.barang') }}" method="GET" class="relative flex-1 max-w-md">
                    ...
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead class="bg-slate-50/75">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 class text-left text-xs font-bold text-slate-400 uppercase tracking-wider">
                                    Deskripsi
                                </th>
                            ...
                            @forelse($categories as $category)
                                <tr class="hover:bg-slate-50/40 transition duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">
                                        {{ $category->name }}
                                        <div class="text-[10px] text-slate-400 font-semibold mt-0.5 tracking-wide">
                                            slug: {{ $category->slug }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold pr-10">
                                        <div class="flex items-center justify-end gap-3.5">
                                            ...
                                            <a href="{{ route('owner.barang.show', $category->slug) }}" 
                                                class="text-blue-600 hover:text-blue-900 transition duration-150 inline-flex items-center gap-1 cursor-pointer">
                                            ...
                                            <!-- Edit Category -->
                                            <a href="{{ route('owner.barang.edit', $category->slug) }}" 
                                            ...
                                            <button type="button" 
                                                @click="deleteActionUrl = '{{ route('owner.barang.destroy', $category->slug) }}'; showDeleteModal = true" 
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        ...
                                        <h4 class="text-sm font-bold text-slate-700">
                                            {{ $search ? 'Hasil pencarian tidak ditemukan' : 'Belum ada kategori' }}
                                        </h4>
                                        <p class="text-slate-400 text-xs mt-1">
                                            {{ $search ? 'Coba masukkan kata kunci pencarian yang berbeda.' : 'Silakan tambahkan kategori barang baru menggunakan tombol di atas.' }}
                                        </p>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <!-- Delete Confirmation Modal (Alpine.js Interactive) -->
    <div x-show="showDeleteModal" 
        ...
        <div @click.away="showDeleteModal = false"
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-base font-extrabold text-slate-800 tracking-tight">Konfirmasi Hapus Kategori</h3>
            </div>
            
            <p class="text-xs text-slate-500 leading-relaxed mb-6">
                Apakah Anda yakin ingin menghapus kategori ini? Kategori ini akan dihapus secara permanen dari sistem.
            </p>
