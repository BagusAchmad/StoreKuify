...
@section('title', 'Tambah Barang Baru - StoreKuify')
...
<div x-data="{ sidebarOpen: false }" class="h-screen flex overflow-hidden bg-slate-50">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <!-- Header -->
        @include('partials.header', ['breadcrumbs' => [['label' => 'Dashboard', 'url' => route('owner.dashboard')], ['label' => 'Data Barang', 'url' => route('owner.barang')], ['label' => $category->name, 'url' => route('owner.barang.show', $category->slug)], ['label' => 'Tambah Barang']]])
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50">
            <div class="max-w-5xl mx-auto space-y-6">
                <!-- Title Header -->
                <div>
                    <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">Tambah Barang Baru</h2>
                    <p class="text-slate-500 text-xs mt-0.5">Masukkan detail barang untuk ditambahkan ke inventaris.</p>
                </div>

                <!-- Two-Column Product Form Box (Alpine.js State calculation for dynamic profit & preview) -->
                <form action="{{ route('owner.products.store', $category->slug) }}" method="POST" enctype="multipart/form-data" 
                    x-data="{ cost: 0, selling: 0, imagePreview: null }"
                    ...
                    <div class="lg:col-span-8 bg-white border border-slate-200/60 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div class="sm:col-span-1">
                                ...
                                <!-- Hidden input to send category_id with the form request -->
