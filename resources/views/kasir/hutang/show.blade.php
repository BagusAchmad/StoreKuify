...
@section('title', 'Detail Hutang - ' . $customer->name . ' - StoreKuify Kasir')
...
<div x-data="{ 
    ...
    get parsedAmount() {
        let val = parseInt(this.paymentAmount.toString().replace(/[^0-9]/g, '')) || 0;
        return val;
    },
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <!-- Header -->
        @include('partials.header', ['breadcrumbs' => [['label' => 'Dashboard', 'url' => route('kasir.dashboard')], ['label' => 'Hutang Pelanggan', 'url' => route('kasir.hutang')], ['label' => $customer->name]]])
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <div class="text-xs font-bold text-slate-400 flex items-center gap-1">
                        ...
                        <a href="{{ route('kasir.hutang') }}" class="hover:text-[#1e5cfb] transition">Hutang Pelanggan</a>
                        <span class="text-slate-300">/</span>
                ...
                <a href="{{ route('kasir.hutang') }}" class="inline-flex items-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 px-4 py-2.5 rounded-xl text-sm font-bold transition duration-150 cursor-pointer">
    ...
    <!-- Bayar Hutang Modal (Matching Owner UI Modal) -->
    <div x-show="showPaymentModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
        <div @click.away="showPaymentModal = false; showQrisStep = false"
            ...
            <div x-show="!showQrisStep" class="p-6 space-y-5">
                ...
                <!-- Hidden Form -->
                <form x-ref="payForm" action="{{ route('kasir.hutang.pay', $customer) }}" method="POST" class="hidden">
                    @csrf
            ...
            <div x-show="showQrisStep" class="p-6 space-y-5" x-cloak>
                ...
                <!-- QRIS Image from Store Settings -->
                <div class="aspect-square w-56 h-56 bg-slate-50 border border-slate-200/80 rounded-2xl mx-auto flex flex-col items-center justify-center p-4">
                    ...
                    @else
                        <div class="text-center">
                            ...
                            <p class="text-[9px] text-slate-400 mt-0.5">Owner belum mengunggah QRIS di Pengaturan > QRIS.</p>
                        </div>
                    @endif
                </div>

                <p class="text-center text-[10px] font-bold text-slate-400">
                    {{ $qrisExists ? 'Minta pelanggan scan QR code di atas untuk membayar.' : 'QRIS harus dikonfigurasi oleh Owner di Pengaturan Toko.' }}
                </p>
